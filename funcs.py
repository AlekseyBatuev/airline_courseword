...
def flights_profitability():
    print("start")
    flights = list(zip(*airline_cur.execute(
        f'SELECT jet_id, arrival_point, ticket_price, number_of_tickets_sold, depreciation_cost, fuel_cost FROM Flights').fetchall()))
    jet_id, airports = flights[0], flights[1]
    models = [airline_bd.execute(f'SELECT model_name FROM "Jets" WHERE jet_id == "{i}"').fetchone()[0] for i in
                jet_id]
    classes = [airline_bd.execute(f'SELECT class_name FROM "Models" WHERE model_name == "{i}"').fetchone()[0] for i
                in models]
    maintenance_cost = [airline_bd.execute(
        f'SELECT maintenance_cost FROM "{airports[i]}" WHERE class == "{classes[i]}"').fetchone()[0] for i in
                        range(len(airports))]
...
def flight_time():
    ...
    travel_times_list = [airline_bd.execute(f'SELECT travel_time FROM Flights WHERE jet_id == "{i}"').fetchall() for
                            i in jet_id]
...
def flight_occupancy():
    print("start")
    flights = list(
        zip(*airline_cur.execute(f'SELECT flight_id, jet_id, number_of_tickets_sold FROM Flights').fetchall()))
    flight_id, jet_id, number_of_tickets_sold = flights[0], flights[1], flights[2]
    models = [airline_bd.execute(f'SELECT model_name FROM "Jets" WHERE jet_id == "{i}"').fetchone()[0] for i in
                jet_id]
    number_of_passangers = [
        airline_bd.execute(f'SELECT number_of_passengers FROM "Models" WHERE model_name == "{i}"').fetchone()[0] for
        i in models]
...
def parking_cost():
    ...
    parking_hours = [
        [parking_cur.execute(f'SELECT "hours" FROM "{i}" WHERE jet_id == "{j}"').fetchall()[0][0] for i in airports]
        for j in jet_id]
    airports_hours_dict = list(dict(zip(airports, i)) for i in parking_hours)
    cost_dict = [
        [airline_bd.execute(f'SELECT parking_cost FROM "{key}" WHERE class == "{classes[i]}"').fetchone()[0] * val
            for key, val in airports_hours_dict[i].items()] for i in range(len(classes))]
